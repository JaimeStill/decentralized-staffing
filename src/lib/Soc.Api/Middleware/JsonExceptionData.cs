...
public class JsonExceptionData
{
    ...
    {
        string body = JsonSerializer.Serialize(this, new JsonSerializerOptions()
        {
            WriteIndented = true
        });

        await File.WriteAllTextAsync(Path.Join(path, GenerateLogName()), body);
    }

    static string GenerateLogName() => $"log-{DateTime.UtcNow:yyyy-MM-dd-HH-mm-ss}.json";
}